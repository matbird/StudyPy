...
def classify_by_ext(target_dir,tmp_dir):
    ...
    for dir_ in os.walk(target_dir):#生3-元组 (dirpath, dirnames, filenames)【文件夹路径, 文件夹名字列表, 文件名列表】
        for f in dir_[2]:
            exts = f.split('.')
    ...
    for ext,file_list in ext_files.items():
        ...
        for file_path in file_list:
            filename = unique_covert(file_path)
            dest = os.path.join(dest_dir,filename)
            rel_file_path = os.path.relpath(file_path,target_dir) #从start(第二个参数)开始计算相对路劲
...
def classify_by_mtime(target_dir,tmp_dir):
    for dir_ in os.walk(target_dir):
        ...
        for f in dir_[2]:
            ...
            if os.path.islink(abs_file_path): # 是否是链接,windows默认返回false
                rel_dest_dir = 'link'
...
def go_back(target_dir):
    ...
    # dirname 获取path的上一层路径,这里相当于回退两步
    ...
    for src,old in back_up_tree.getItems():
    ...
    shutil.rmtree(target_dir,ignore_errors=False) # 移除目录树
...
if __name__ == '__main__':
    # print(unique_covert('C:\\test\\aaa\\bb.txt'))
    # save_backup_gen = save_back_up(os.getcwd())
    # save_backup_gen.send((1,2))
    # save_backup_gen.send(None)
    _main()
    pass
